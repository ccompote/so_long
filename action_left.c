...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/01/13 21:08:32 by ccompote          #+#    #+#             */
/*   Updated: 2023/01/13 21:23:59 by ccompote         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "so_long.h"

void	left_c(t_game *zop, int x_temp, int y_temp)
{
	...
	zop->rat_image = mlx_texture_to_image(zop->mlx, zop->rat_links_texture);
	mlx_image_to_window(zop->mlx, zop->rat_image, x_temp, y_temp);
	zop->points_end++;
}

void	left_move(t_game *zop, int x_temp, int y_temp)
{
	mlx_delete_image(zop->mlx, zop->rat_image);
	zop->rat_image = mlx_texture_to_image(zop->mlx, zop->rat_links_texture);
	zop->direction = 1;
...
void	action_left(mlx_key_data_t keydata, void *param)
{
	...
	if (keydata.action == MLX_RELEASE)
	{
		if (zop->map[(y_temp) / 32][(x_temp - 32) / 32] == 'C')
			left_c(zop, x_temp, y_temp);
		if (zop->map[(y_temp) / 32][(x_temp - 32) / 32] != '1')
		{
			if (zop->direction != 1)
				left_move(zop, x_temp, y_temp);
